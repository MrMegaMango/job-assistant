...
Job Assistant stores its SQLite database outside the Git checkout. On Linux and WSL the default is `~/.local/share/job-assistant`; on Windows it uses `%LOCALAPPDATA%\JobAssistant`.

On Vercel, the app runs only as a disposable public demo. Its temporary database lives under `/tmp`, may reset at any time, and the server rejects profile and application-state changes. Do not enter personal data into a hosted demo or remove those guards without first adding authentication and durable private storage.
